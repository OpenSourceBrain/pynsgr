...
class Client(object):
    ...
    def __init__(
        self, appname, appID, username, password, baseUrl, endUserHeaders=None
    ):
        ...
        if endUserHeaders:
            ...
            if not self.endUsername or not endUserHeaders.get("cipres-eu-email"):
                raise Exception(
                    "endUserHeaders must include cipres-eu and cipres-eu-email"
                )
    ...
    def listJobs(self):
        ...
        r = self.__doGet__(
            url=self.baseUrl + "/job/" + self.endUsername + "/?expand=true"
        )
    ...
    def getJobStatus(self, jobHandle):
        ...
        return JobStatus(
            client=self,
            jobUrl=self.baseUrl + "/job/" + self.endUsername + "/" + jobHandle,
        )
    ...
    def submitJobTemplate(self, testdir, metadata=None, validateOnly=False):
        ...
        with open(testdir + "testInput.properties") as infile:
            fileParams.load(infile)
        otherParams = Props.Properties()
        with open(testdir + "testParam.properties") as infile:
        ...
        return self.submitJob(
            otherParams.getPropertyDictAsList(),
            fileParams.getPropertyDict(),
            metadata=metadata,
                ...
                if element.tag == "error":
                    ...
                    if cipresCode == 5:
                        for fieldError in element.findall("paramError"):
                            fieldErrors[
                                fieldError.find("param").text
                            ] = fieldError.find("error").text
    ...
    def __doDelete__(self, url):
        ...
        r = requests.delete(
            url, auth=(self.username, self.password), verify=False, headers=self.headers
        )
        ...
        if xml.find("messages") is not None:
            for m in xml.find("messages"):
                self.messages.append(
                    "%s: %s" % (m.find("timestamp").text, m.find("text").text)
                )
...
class Application(object):
    ...
    def __init__(self, conf_filepath=None):
        ...
        if not requiredProperties.issubset(self.props.propertyNames()):
            raise Exception(
                "%s doesn't contain all the required properties: %s"
                % (CONF_FILENAME, ", ".join(requiredProperties))
            )
